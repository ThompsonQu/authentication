...
void decryption(const unsigned char* key,
                size_t nkey,
                FILE* message,
                FILE* ciphertext) {
    unsigned char ch;
    size_t i = 0;
    while (1) {
        fread(&ch, 1, 1, ciphertext);
        if (feof(ciphertext)) {
            break;
        }
        int dec = ch;
        dec = (dec - key[i] + GRIDSIZE) % GRIDSIZE;
        unsigned char d = dec;
        i = (i + 1) % nkey;
        fwrite(&d, 1, 1, message);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    FILE *message = fopen(msg_file, "wb");
    if (!message) {
        fprintf(stderr, "cannot open message file %s\n", argv[2]);
        return EXIT_FAILURE;
    }

    FILE *ciphertext = fopen(cipher_file, "rb");
    if (!ciphertext) {
    ...
    decryption(key, nkey, message, ciphertext);
    if (key_file != NULL) {
        free(key);
    }
}
